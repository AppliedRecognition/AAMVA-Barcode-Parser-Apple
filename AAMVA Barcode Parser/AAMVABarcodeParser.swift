...
//                    }
                ...
                docData.setValue(DataField(description: "State/Province", originalValue: stateProvince, parsedValue: stateProvince), forField: "State/Province")
                let city = string(&track, toDelimiter: "^", maxLength: 13)
                docData.setValue(DataField(description: "City", originalValue: city, parsedValue: city), forField: "City")
                let name = string(&track, toDelimiter: "^", maxLength: 35)
                let names = name.components(separatedBy: "$")
                if names.count > 2 {
                    docData.setValue(DataField(description: "Title", originalValue: names[2], parsedValue: names[2].trimmingCharacters(in: CharacterSet(charactersIn: ",/ "))), forField: "Title")
                }
                if names.count > 1 {
                    docData.setValue(DataField(description: "First name", originalValue: names[1], parsedValue: names[1].trimmingCharacters(in: CharacterSet(charactersIn: ",/ "))), forField: "First name") 
                }
                if names.count > 0 {
                    docData.setValue(DataField(description: "Last name", originalValue: names[0], parsedValue: names[0].trimmingCharacters(in: CharacterSet(charactersIn: ",/ "))), forField: "Last name") 
                }
                let address = string(&track, toDelimiter: "^", maxLength: 77-name.count-city.count).components(separatedBy: "$").joined(separator: "\n")
                docData.setValue(DataField(description: "Address", originalValue: address, parsedValue: address), forField: "Address") 
                if track.first! == "?" {
                    track.removeFirst()
                }
                if let idx = track.firstIndex(of: ";") {
                    ...
                    docData.setValue(DataField(description: "IIN", originalValue: iin, parsedValue: iin), forField: "IIN") 
                    ...
                    if let date = expiryDate {
                        docData.setValue(DataField(description: "Date of expiry", originalValue: originalDate, parsedValue: date), forField: "Date of expiry") 
                    }
                    docData.setValue(DataField(description: "Date of birth", originalValue: birthYear+birthMonth+birthDate, parsedValue: birthYear+"/"+birthMonth+"/"+birthDate), forField: "Date of birth") 
                    dlNo += string(&track, toDelimiter: "=", maxLength: 5)
                    docData.setValue(DataField(description: "DL/ID#", originalValue: dlNo, parsedValue: dlNo), forField: "DL/ID#") 
                    if let idx = track.firstIndex(of: "%") {
                        track.removeFirst(idx.utf16Offset(in: track)+1)
                        let versionNumber = remove(1, charactersFromString: &track)
                        docData.setValue(DataField(description: "Version #", originalValue: versionNumber, parsedValue: versionNumber), forField: "Version #") 
                        let securityVersionNumber = remove(1, charactersFromString: &track)
                        docData.setValue(DataField(description: "Security v. #", originalValue: securityVersionNumber, parsedValue: securityVersionNumber), forField: "Security v. #") 
                        let postalCode = remove(11, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        docData.setValue(DataField(description: "Postal code", originalValue: postalCode, parsedValue: postalCode), forField: "Postal code") 
                        let dlClass = remove(2, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !dlClass.isEmpty {
                            docData.setValue(DataField(description: "Class", originalValue: dlClass, parsedValue: dlClass), forField: "Class") 
                        }
                        let restrictions = remove(10, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !restrictions.isEmpty {
                            docData.setValue(DataField(description: "Restrictions", originalValue: restrictions, parsedValue: restrictions), forField: "Restrictions") 
                        }
                        let endorsements = remove(4, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !endorsements.isEmpty {
                            docData.setValue(DataField(description: "Endorsements", originalValue: endorsements, parsedValue: endorsements), forField: "Endorsements") 
                        }
                        let sex = remove(1, charactersFromString: &track)
                        docData.setValue(DataField(description: "Sex", originalValue: sex, parsedValue: sex), forField: "Sex") 
                        let height = remove(3, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !height.isEmpty {
                            docData.setValue(DataField(description: "Height", originalValue: height, parsedValue: height), forField: "Height") 
                        }
                        let weight = remove(3, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !weight.isEmpty {
                            docData.setValue(DataField(description: "Weight", originalValue: weight, parsedValue: weight), forField: "Weight") 
                        }
                        let hairColor = remove(3, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !hairColor.isEmpty {
                            docData.setValue(DataField(description: "Hair color", originalValue: hairColor, parsedValue: AAMVABarcodeParser.hairColour(hairColor)), forField: "Hair color") 
                        }
                        let eyeColor = remove(3, charactersFromString: &track).trimmingCharacters(in: .whitespaces)
                        if !eyeColor.isEmpty {
                            docData.setValue(DataField(description: "Eye color", originalValue: eyeColor, parsedValue: AAMVABarcodeParser.eyeColour(eyeColor)), forField: "Eye color") 
                        }
                    }
                }
                if docData.isEmpty {
